...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize audio: start paused on page load
    audio.muted = false;
    audio.volume = 0.5;
    audio.pause();  // Start paused
    let musicStarted = false;
    ...
    const updatePage1ButtonIcon = () => {
        if (page1ToggleButton) {
            if (isPaused()) {
                ...
                page1ToggleButton.title = 'Play background music';
            } else {
                page1ToggleButton.innerHTML = '<i class="fas fa-volume-up"></i>';
                page1ToggleButton.title = 'Pause background music';
            }
        }
    };
    ...
    const updatePage2ButtonIcon = () => {
        if (page2ToggleButton) {
            if (isPaused()) {
                ...
                page2ToggleButton.title = 'Play background music';
            } else {
                page2ToggleButton.innerHTML = '<i class="fas fa-volume-up"></i>';
                page2ToggleButton.title = 'Pause background music';
            }
        }
    };
    ...
    // 1. Page 1 Music Toggle Function (Pause/Play)
    ...
    // 2. Page 2 Music Toggle Function (Pause/Play)
    ...
    if (entranceButton) {
        entranceButton.addEventListener('click', () => {
            // Unpause/play music when decorative button is clicked
            if (audio && audio.paused) {
                audio.play().catch(e => console.log("Play error on entrance:", e));
                musicStarted = true;
                updatePage2ButtonIcon();
                console.log("Music unpaused successfully");
            }
        ...
        // Initialize page 1 button icon (show mute since audio is paused)
        updatePage1ButtonIcon();
    } else {
    ...
    // Start with music paused on page load
    const startAudio = () => {
        if (!audio) return;
        ...
        audio.pause();
        console.log("Audio loaded and paused - ready for user interaction");
    };
